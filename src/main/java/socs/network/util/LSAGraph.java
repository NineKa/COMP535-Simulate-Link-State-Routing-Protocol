...
public class LSAGraph {
    ...
    public synchronized void build_graph(HashMap<String, LSA> raw_table) {
        ...
        for (String key : raw_table.keySet()) {
            for (LinkDescription l : raw_table.get(key).links) {
                if (get_link(node_table.get(key), node_table.get(l.linkID)) == null) {
                    link_table.put(
                            key + l.linkID,
                            new lsa_link(node_table.get(key), node_table.get(l.linkID), l.linkWeight)
                    );
                    node_table.get(key).add_link(link_table.get(key + l.linkID));
                    node_table.get(l.linkID).add_link(link_table.get(key + l.linkID));
                }
            }
        }
    }
    ...
    public synchronized String get_shortest_path(String source_id, String target_id) {
        ...
        for (; ; ) {
            if (current == null) return "unable to establish path";
            list.add(current);
            if (current == source) {
                break;
            }
